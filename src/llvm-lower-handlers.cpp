 ...
 */
static void ensure_enter_function(Module &M, const Triple &triple)
{
...
static bool lowerExcHandlers(Function &F) {
    ...
    ensure_enter_function(M, triple);
    ...
    for (auto it : EnterDepth) {
        ...
        if (triple.isOSWindows()) {
            ...
            sj = CallInst::Create(setjmp_func, ArrayRef<Value*>{ buff, ConstantInt::getNullValue(Type::getInt32Ty(M.getContext())) }, "", enter);
        }
