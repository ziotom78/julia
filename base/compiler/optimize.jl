...
struct InlineeMetaInfo
    rt
    mi::MethodInstance
    InlineeMetaInfo(@nospecialize(rt), mi::MethodInstance) = new(rt, mi)
end

"""
    is_inlineable(interp::AbstractInterpreter, src::MaybeCompressed,
        metainfo::Union{InlineeMetaInfo,Nothing}=nothing) -> Bool
...
If `metainfo::InlineeMetaInfo` is available, take it into account to add extra inlining willingness.
"""
function is_inlineable(interp::AbstractInterpreter, @nospecialize(src::MaybeCompressed),
    metainfo::Union{InlineeMetaInfo,Nothing}=nothing)
...
struct InlineeInfo
    ...
    info::CallInfo
    stmt_flag::UInt8
    function InlineeInfo(@nospecialize(rt), mi::MethodInstance, argtypes::Vector{Any},
...
"""
    inlining_policy(interp::AbstractInterpreter, src, iinfo::InlineeInfo) -> newsrc
...
"""
function inlining_policy(interp::AbstractInterpreter, @nospecialize(src), iinfo::InlineeInfo)
    if isa(src, MaybeCompressed)
        if is_source_inferred(src)
            if is_stmt_inline(iinfo.stmt_flag) || is_inlineable(interp, src, InlineeMetaInfo(iinfo.rt, iinfo.mi))
                return src
            end
        end
...
function finish(interp::AbstractInterpreter, opt::OptimizationState,
    ...
    if (isa(result, Const) || isconstType(result))
        ...
        if proven_pure
            ...
            force_noinline || (src.inlining_cost = DEFAULT_INLINEABLE_COST)
        end
    end
    ...
    elseif isa(def, Method)
        if is_declared_inline(src) && isdispatchtuple(specTypes)
            ...
            src.inlining_cost = inlining_cost(ir, params, union_penalties)
        end
    end
